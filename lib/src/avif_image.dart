...
class AvifImageState extends State<AvifImage> with WidgetsBindingObserver {
  ...
  void didUpdateWidget(AvifImage oldWidget) {
    ...
    if (widget.image != oldWidget.image) {
      avif_ffi.api.disposeDecoder(key: oldWidget.image.hashCode.toString());
      _resolveImage();
    }
  }
...
class AvifCodec {
  ...
  AvifCodec({
    ...
    try {
      avif_ffi.api
          .initMemoryDecoder(key: _key, avifBytes: avifBytes)
          .then((info) {
  ...
  String? _getNextFrame(void Function(ui.Image?, int) callback) {
    try {
      avif_ffi.api.getNextFrame(key: _key).then((frame) {
  ...
  void dispose() {
    avif_ffi.api.disposeDecoder(key: _key);
  }
}
